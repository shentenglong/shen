...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			$(function () {
				$('#txtUsername').mouseleave(function () {
					var obj=$('#txtUsername').val();
					if(obj.trim()==""){
					    $('#txtUsernameValidMsg').html('email不能为空').css('color','red')
					}
					else {
                        $('#txtUsernameValidMsg').html('')
					}
                })
                $('#txtPassword').mouseleave(function () {
                    var obj=$('#txtPassword').val();
                    if(obj.trim()==""){
                        $('#txtPasswordValidMsg').html('密码不能为空').css('color','red')
                    }
                    else {
                        $('#txtPasswordValidMsg').html('')
                    }
                })
            })
		</script>
						...
						<form method="post" action="login.action" id="ctl00">
							<ul>
								<li>
									...
									<div class="text_left" id="txtUsernameValidMsg"></div>
								</li>
								...
								<li>
									...
									<input type="password" name="password" id="txtPassword"
										class="textbox" />
									<div class="text_left" id="txtPasswordValidMsg"></div>
								</li>
								<li>
